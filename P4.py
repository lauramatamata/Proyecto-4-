...
def bits_a_rgb(bits_Rx, dimensiones):
    '''Esta es una función que descodifica los bits
    recuperados del proceso de demodulación
...
def modulador(bits, fc, mpp):
    ...
    Tc = 1 / fc  # periodo [s]
    t_periodo = np.linspace(0, Tc, mpp)  # mpp: muestras por período
    #Se definen cada uno de los casos para la portadora, el primero sería para el caso con coseno y el segundo caso con seno.
    ...
    j = 0 # Conteo de muestreo
    
    for i in range(0, N, 4): #  Donde bit_1 = bits[i], bit_2 = bits[i+1], bit_3 = bits[i+2] y bit_4 = bits[i+3]
        # bit_1 = bits[i], bit_2 = bits[i+1] define A1. bit_3 = bits[i+2] y bit_4 = bits[i+3] define A2
        
        senal_Tx[j*mpp: (j+1)*mpp] = (-1)**(1+bits[i]) * 3**(1-bits[i+1]) * portadora_cos + (-1)**(bits[i+2]) * 3**(1-bits[i+3]) * portadora_sin
        j += 1
...
def demodulador(senal_Rx, portadora_cos, portadora_sin, mpp):
 # Cantidad de muestras en senal_Rx
    M = len(senal_Rx)
 ...
 #Pseudo-Energía de un período de la portadora:
    ...
    for i in range(N):
 #Se iguala la cantidad de iteraciones que puede correr el programa y la cantidad de información que se puede recuperar
        if j+4 > N:  
            break   
 ...
 # Para asignar signos:
        if np.sum(producto_cos) >= 0:
            bits_Rx[j] = 1  # Para el bit 1
     
        if np.sum(producto_sin) < 0:
            bits_Rx[j+2] = 1  # Para el bit 3

        # Ahora se procede a calcular la magnitud 
        if np.max(np.abs(producto_cos)) < 3:
            bits_Rx[j+1] = 1  # Para el bit 2
        if np.max(np.abs(producto_sin)) < 3:
            bits_Rx[j+3] = 1  # Para el bit 4
        j += 4
    return bits_Rx.astype(int), senal_demodulada
...
senal_Tx, Pm, portadora_cos, portadora_sin = modulador(bits_Tx, fc, mpp)

# 4. Se transmite la señal modulada, por un canal ruidoso
senal_Rx = canal_ruidoso(senal_Tx, Pm, SNR)

# 5. Se desmodula la señal recibida del canal
bits_Rx, senal_demodulada = demodulador(senal_Rx, portadora_cos, portadora_sin, mpp)
